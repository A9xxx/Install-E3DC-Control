...
- **Preisabhängige PV-Ladung:** Die PV-Ladeleistung in Eco+ berücksichtigt günstige Verkaufspreise auch ohne Verkaufs- oder Negativpreisfenster. Die bestehende Sicherheitsladekurve, DC-Quellentrennung, Reserven und Abregelschutz bleiben bindend. Ein vollständiger Ladeaufschub bis zur billigsten Stunde ist nicht enthalten.
...
- **Wattvertrag vor Treiberübersetzung:** Die zentrale Wallbox-Regelung bindet je Ladepunkt verfügbare Quelle, physischen Stromschritt, Fahrzeug- und Infrastrukturgrenzen sowie den endgültigen Aktorauftrag. Der Treiber übersetzt diesen Auftrag in das jeweilige Geräteprotokoll und trifft keine zweite Lade- oder Preisentscheidung.
- **Schnell hoch, kontrolliert herunter:** Ein vollständig finanziertes höheres Stromziel wird nicht durch eine zweite nachgelagerte 1-A-Rampe verzögert. Bei Unterdeckung folgt die Kaskade weiterhin der Physik: erst Strom reduzieren, bei einer dafür belegten umschaltbaren Wallbox gegebenenfalls die Phase wechseln und erst am einphasigen Mindeststrom stoppen.
- **Gerätefähigkeiten werden nicht geraten:** Direkt kommandierbare Phasen, autonome Herstellerumschaltung, feste Topologie, Stromgrenzen und frische Istwerte bleiben getrennte Belege. Ein unbekannter Wert erzeugt weder eine erfundene Fähigkeit noch zusätzliche Ladeleistung.
- **openWB Pro behält die Gerätefolge:** Der Phasenwechsel folgt `0 A`, frischem Ruhenachweis, Phasenziel, kurzer Ziel-/Wiederanlaufwartezeit und erst danach erneuter Stromfreigabe. Das Gerätekommando für das Phasenziel übernimmt die fahrzeugseitige Pause. CP-Wake-up bleibt ein getrenntes, fahrzeugprofilgebundenes Startverfahren und ist standardmäßig aus. Die 480-Sekunden-Frist sperrt nur den nächsten Phasenwechsel, nicht die laufende Ladung.
- **Nativer E3/DC-Status bleibt lesend gebunden:** Frische, im RSCP-Client definierte Wallboxwerte liefern Stecker- und Statusbelege, die aktuelle Phasenkonfiguration, typbestätigte Leistung sowie rohe Diagnosefelder. Aus `NUMBER_PHASES` oder `PARAM_1` werden weder Umschaltfähigkeit noch bestätigter Ladestrom erfunden. Ein Diagnose- oder Altpfad sendet keinen konkurrierenden Hardwarebefehl.

### ⚡ Netzdefizit und Energiequellen werden genau einmal bewertet

- **Ein Netz-Wh-Konto für die Wallboxgruppe:** Derselbe frische Netzpunkt-Snapshot wird gruppenweit genau einmal integriert. Echter Netzbezug hat Vorrang; eine gleichzeitige Budgetüberziehung bleibt dann Diagnose und beschleunigt den Zähler nicht zusätzlich.
- **PV-Kurve und PV + Akku bleiben getrennt:** Jede Wallbox erhält nur den zu ihrem Modus gehörenden Quellenrahmen. Batterieneutrale PV-Leistung kann keine Akkuentladung autorisieren; die ausdrücklich freigegebene Akkuunterstützung bleibt an ihre SoC-Untergrenze und Schutzgrenzen gebunden.
- **Kurvenladung gewinnt am Kurvenstart:** Sobald der aktuelle Speicher-SoC unter dem wirksamen Kurvenziel liegt, endet der Pre-Dump-Vorrang. Der Speicher darf den nachfolgenden PV-Ertrag wieder für seine normale Kurvenladung nutzen, statt ihn weiter für eine bereits abgelaufene Entladephase freizuhalten.
- **Pre-Dump-Pause erhält PV-Laden:** Eine geschlossene oder pausierte Pre-Dump-Akkufreigabe sperrt ausschließlich zusätzliche Batterieunterstützung. Frisch belegte batterieneutrale PV-Leistung bleibt für eine angeschlossene Wallbox nutzbar und erzeugt weder Netz- noch Akkuentladefreigabe.
- **Keine Scheinreservierung flexibler Verbraucher:** Nur reale Last und ein tatsächlich gebundener Startvorgang belegen Leistung. Ein bloßer Wärmepumpen-Startwunsch hält kein dauerhaftes Wallboxbudget zurück.
- **Wallboxbudget bleibt bei der Wallbox:** Eine ausschließlich für Fahrzeugladung freigegebene Batteriequelle darf keinen neuen Wärmepumpen- oder Heizstabstart finanzieren. Bereits laufende Lasten und ein zuvor vollständig gebundener Wärmepumpenstart behalten ihren Schutzvorrang vor einer neuen Wallboxzuteilung.
- **Frische Messgeneration statt geratenem Nullwert:** JSON-Snapshots werden nach dem Lesen gegen dieselbe Dateigeneration bestätigt. Ein Austausch während des Lesens, ein Last-Good-Wert oder ein veralteter Zustand darf weder neues Budget noch einen Hardwareentscheid erzeugen.

### 🔋 E3/DC-DC-Leistung bleibt bei offener Hausversorgung nutzbar

- **DC-first auch nach einer alten Begrenzung:** Ist die Speicherentladung offen und liegt eine frische, gültige E3/DC-DC-Leistung vor, darf eine zuvor bestätigte niedrige Ladegrenze kontrolliert bis zu diesem internen DC-Rahmen gelöst werden. Leistung eines zusätzlichen AC-Wechselrichters wird dabei nicht als E3/DC-DC-Leistung umgedeutet.
- **Kein blindes Öffnen:** Ungültige oder veraltete Livewerte, widersprüchliche POWER_SETTINGS-Rückmeldungen, Netzbezug und harte Speichergrenzen verhindern die Freigabe. Preis-, Direktvermarktungs- und ausdrücklich freigegebene Netzladeverträge bleiben eigenständig.

### 📊 Ehrliche Wallbox- und Prognoseanzeige

- **Kein künstliches Nachlaufen der Ladeleistung:** Das Dashboard projiziert nur eine frische kanonische Leistungsmessung. Stecker, Verriegelung, Sollstrom, Phasensumme oder ein alter Cache halten nach einem bestätigten Stopp keine frühere kW-Leistung mehr sichtbar.
- **Keine Startfreigabe ohne Fahrzeug:** Eine alte Start- oder Budgetprojektion bleibt nach dem Abstecken nicht als aktuelle Ladefreigabe sichtbar. Fehlender Stecker, tatsächlicher Ausgang und Messleistung gewinnen vor einem veralteten Sollzustand.
- **Beide Wallboxen in der Tagesstatistik:** WB1 und WB2 führen getrennte Sitzungs- und Tageszähler. Mitternacht, Abstecken und Zählerreset werden pro Ladepunkt gebunden; die Tagesansicht summiert beide Wallboxen, ohne eine laufende Sitzung doppelt zu zählen.
- **PV-Prognosediagnose auf allen Oberflächen:** Die rein lesende Auswertung wird aus einer gemeinsamen Quelle in Installationszentrale, Desktop- und Mobilansicht dargestellt. Verfügbar, deaktiviert und noch ohne abgeschlossene Evidenz sind eindeutige Zustände; Diagnosewerte greifen nicht in Modell oder Regelung ein.
- **Planidentische SoC-Projektion:** Die Standard-SoC-Prognose stammt ausschließlich aus `projection.soc_pct` des aktuellen kanonischen Plans und seiner Slots. Ziel-, Unter-/Oberkurve und Zwischenanker werden nicht als Prognose umgedeutet. Fehlen beim selben Plan Projektionswerte, bleiben diese Zielhilfen sichtbar; bei einer neuen Planrevision wechselt das Frontend das gesamte Kurvenbündel atomar, ohne alte und neue Slots zu mischen.
- **Simulator-Tagesgrenze bleibt vollständig:** Liefert ein kompatibler Speicherplan eine ausdrückliche, von Mitternacht abweichende Anzeigegrenze, umfasst die Kurve ab dort den vollständigen folgenden lokalen Tag. Eine verschobene Grenze kann die SoC- und Ertragsprojektion nicht mehr auf die verbleibenden Stunden bis Mitternacht verkürzen.
- **Regelruhe und Datenqualität getrennt:** Reine Entscheidungsweg- oder Sollwertwechsel gelten nicht als Takten. Ein Speicherausgang zählt erst mit einem kohärenten, bestätigten RSCP-Write samt frischem POWER_SETTINGS-Readback oder einem eindeutig beibehaltenen kanonischen Readback. Unterdrückte Sollwechsel und unbestätigte Teil-Writes bleiben `EVIDENCE_LIMIT`; echte Ausgangswechsel sowie Veto-/Pfadkonflikte bleiben sichtbar. Ein kurzer einzelner Plausibilitäts-Guard erscheint nur als Datenqualitätshinweis.
- **Historie respektiert die Leistungs-Deadband:** Ein fachlicher Wechsel der Speicher-Ausführungsklasse bleibt sofort sichtbar. Kleine Sollwertschritte innerhalb der bestehenden 500-W-Grenze erzeugen dagegen keinen zusätzlichen persistenten Historieneintrag; bestätigte reale Ausgangswechsel werden weiterhin unabhängig erfasst.
- **Ein Frontendvertrag:** Quell- und minifizierte JavaScript-Datei werden gemeinsam geprüft und über den regulären Update- und Rechteumfang ausgeliefert.

### ♨️ Luxtronik-Warmwasserstatus folgt dem realen Ablauf

- **Anforderung, Hydraulik und Verdichter getrennt:** Die Anzeige unterscheidet eine Warmwasseranforderung, bestätigte BUP-/WW-gebundene ZUP-Hydraulik, den gestarteten Verdichter, die gemessene 40-Hz-Zwischenstufe und das Erreichen der angeforderten Verdichter-Ziellast.
- **Keine Leistung aus Pumpensignalen erfunden:** BUP oder ZUP allein beweisen weder einen Verdichterstart noch volle Leistung. Fehlende, veraltete oder widersprüchliche Werte bleiben unbekannt. Die neue Stufenanzeige ist rein lesend und verändert weder Wärmebudget noch Sollwerte oder Schutzzeiten.

### 🔄 Erfolgreiches Update endet ohne nachgelagerten Shellfehler

- **Worker-Scope bleibt bis zur Bereinigung gültig:** Der reguläre Abschluss ruft die Bereinigung noch innerhalb der laufenden Funktion auf. Unter `set -u` entsteht danach keine Meldung `worker_cleanup_started: unbound variable` mehr.
- **Fehler- und Signalpfad bleibt erhalten:** Frühere Fehler und Signale werden weiterhin durch den bestehenden EXIT-Trap verarbeitet; Backup-, Rechte-, Rückfall- und Dienstprüfungen werden nicht gelockert.
- **Config-Lock bleibt nach Update und Speichern gültig:** Stable-Updater, Rechteprüfung und Web-Publisher verwenden für den festen Konfigurations-Lock denselben Eigentümer- und Modusvertrag. Wiederholtes Speichern von Ankerkurve, Heizstab-Automatik oder anderen V4-Werten endet dadurch nicht mehr nach einem ersten erfolgreichen Schreibvorgang mit `lock_metadata_invalid`.
- **Prüfen und Reparieren sind eindeutig getrennt:** „Nur Rechte prüfen“ liest ausschließlich. „System reparieren“ benennt vor dem Start ausdrücklich Backup, vollständigen Stable-Abgleich, Rechteprojektion, Dienstneustart und die mögliche erneute Installation derselben Version.
- **Lokales Git ist keine Root-Autorität:** Die Konsolen-Rechteprüfung darf Abweichungen der Checkout-Wrapper nur diagnostizieren. Root-eigene Launcher und `sudoers` werden ausschließlich innerhalb einer commitgebundenen Release-Transaktion ersetzt; außerhalb davon verweist die Prüfung auf Stable-Systemreparatur oder, bei defektem Root-Launcher, auf den veröffentlichten Bootstrap.
- **Web-Neustart ist kein Updatefehler:** Während der kontrollierten Apache-Unterbrechung zeigt die Oberfläche einmalig einen verständlichen Betriebsstatus statt wiederholter HTTP-502-Zähler. Der Installationscenter-Poller wartet wie das Dashboard bis zu zwei Minuten auf die Rückkehr und erklärt danach einen unbekannten Anzeigestatus, ohne den unabhängigen Systemjob fälschlich als fehlgeschlagen zu melden.
- **Abschluss gehört zum gestarteten Lauf:** Der root-eigene Dispatcher veröffentlicht für jeden neuen Systemjob erst nach vorbereitetem Protokoll und eindeutigem Startstatus eine zufällige Laufkennung. Dashboard und Installationszentrale akzeptieren Erfolg oder Fehler nur für genau diesen Lauf; ein alter globaler Abschluss kann nicht mehr als Ergebnis eines neuen Klicks erscheinen. Wechselt die Kennung während einer Statusabfrage, wird der gemischte Zwischenstand verworfen. Auch der erste Lauf ohne vorhandene Ausgangskennung bleibt eindeutig zuordenbar. Jede einzelne Statusanfrage besitzt zusätzlich ein festes Zeitlimit.
- **Root-Launcher bleibt vom Checkout getrennt:** Außerhalb des Release-Updaters prüft die Rechteprüfung den bereits installierten Service-Launcher ausschließlich auf root-eigene Metadaten sowie seine feste Aktions- und Dienstliste. Ein lokaler Entwicklungsstand kann weder einen falschen Root-Fehler erzeugen noch privilegierte Inhalte vorgeben.
- **Heizstab-Speichern meldet die wirkliche Phase:** Scheitert die Heizstab-Automatik bereits am Config-Lock, nennt die Oberfläche Fehlercode und unveränderten Bestand ausdrücklich; ein solcher Speicherfehler wird nicht als Defekt der eigentlichen Heizstabregelung ausgegeben.
- **Heizstab-Automatik liest kompatible Wahrheitswerte:** Manager, Heizstabseite und Konfigurationseditor unterscheiden echte JSON-Wahrheitswerte sowie die bereits projektweit zulässigen textuellen Ein-/Aus-Formen einheitlich. Eine manuell erzeugte, gültige V4-Konfiguration schaltet die Automatik dadurch weder unbeabsichtigt aus noch übergeht sie ein ausdrücklich gespeichertes `false`.
- **Reine Rechtereparatur ohne Update:** Die Installationszentrale kann Besitzer, Gruppe und Modus ausschließlich für die versiegelte Liste bekannter Produkt- und Laufzeitpfade korrigieren. Dieser Weg erstellt kein Vollbackup, ersetzt keine Inhalte und startet keine Dienste neu; unbekannte Dateien bleiben unangetastet.
- **Lokale Inhaltsänderungen werden vollständig gebunden:** Die reine Rechtereparatur korrigiert releasegleiche Einträge sofort und lässt lokal veränderte Dateien zunächst unangetastet. Deren vollständige Pfadliste wird mit Vertrags-Digest und Fingerprints an ein fünf Minuten gültiges Einmal-Token gebunden, das nur über stdin bestätigt werden kann. Die Stable-Systemreparatur besitzt unabhängig davon ihre eigene Zielrelease-, Dateilisten- und Fingerprintbindung und prüft sie nach Writer-Ruhe unmittelbar vor dem Dateiaustausch erneut.
- **Emergency-Latch bleibt über Update und Rückfall wirksam:** Ein aktiver Storage-Notfall-Latch hält den Storage Writer auch bei Zielstart, fehlgeschlagenem Update, Rücklauf auf eine ältere Unit und Neustart gesperrt. Der root-eigene systemd-Startschutz wird versionsunabhängig projiziert; ohne aktiven Latch bleibt der normale Startpfad unverändert.
