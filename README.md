...
Ein hochperformantes, modulares Dashboard und Installations-System für die **native Python-Architektur** [A9xxx/Install-E3DC-Control](https://github.com/A9xxx/Install-E3DC-Control) <kbd>Version 5.4.4f</kbd>. Es verwandelt das System in ein intelligentes Smart-Home-Zentrum mit moderner Web-Oberfläche, eigenem Energy Manager und proaktivem Systemschutz.

![E3DC-Control Dashboard](html/app-icon-512.png)

## Aktuelle Version und Update

Die aktuelle stabile Version ist **5.4.4f**. Hinweise zum Web-, Konsolen- und Docker-Update sowie zur Wiederherstellung stehen in [doc/Update.md](doc/Update.md). Der sanitierte Root **v5.3.2b** bleibt ausschließlich als Docker-Rückfall-Image verfügbar. Ein Bare-Metal-Programm-Rückfall auf diesen Stand wird nicht angeboten; dort bleibt die Wiederherstellung aus einem verifizierten Datei-Backup der sichere Rückweg.
...
> **Neu in 5.4.4f Stable:** Die ruhige PV-Kurve berücksichtigt belegten physischen PV-Überschuss bereits beim Start eines einphasigen Fahrzeugs, ohne Speicher-, Wallbox-, Hausanschluss-, Fahrzeug- oder Hardwaregrenzen zu lockern und ohne Netzladen oder zusätzliche Batterieentladung freizugeben. Das sichtbare Wallboxbudget ist an Regelzyklus, Modus und Revision gebunden: Ein unvollständiger Übergang erscheint als `--`, während ein bestätigtes Sicherheits- oder Regelbudget von `0 kW` sofort sichtbar bleibt. Die Gerätefamilie `Multi` gilt nicht mehr als Beleg für eine zweite Wallbox. Ein ausdrücklicher WB2-Aus-Wert wie `none` bleibt ein hartes Veto; fehlende und historisch leere Typfelder bleiben dagegen unverändert Legacy-kompatibel. Eine zweite openWB wird daraus ausschließlich nach frischer direkter Erkennung zweier verschiedener positiver Ladepunkt-IDs, bestätigtem Status und eindeutig verschiedenen physischen Befehlsausgängen nicht persistent eingebunden; bis dahin bleibt sie vollständig planungs-, zuteilungs- und schreibgesperrt. Der gemeinsame Energiefluss blendet den Solar-Summenpunkt auf Desktop und Mobil dynamisch ein, sobald ein Zusatzwechselrichter kohärent belegt ist, ohne den externen Anteil doppelt zu addieren oder gespeicherte Nutzerpositionen zu überschreiben. Speicher- und Direktvermarktungsansicht benennen fehlende, unvollständige oder planfremde SoC-Prognosen und unterscheiden ersetzende Aktionen als geplant, angefordert oder bestätigt wirksam. Der Zielbestand beträgt maximal drei automatisch verwaltete System-Backup-Familien und separat maximal drei Web-Installer-Sicherungen; Schutzbindungen dürfen diese Zielgrenzen vorübergehend überschreiten, ohne einen Abbruch oder eine erzwungene Löschung auszulösen. Der private Downloadbereich bleibt geschützt, während der Ziel-Updater den betriebenen Produktbaum mit expliziten Live-Rechten veröffentlicht und dessen Installationspfad vor dem Dienststart real als `www-data` prüft. Direkter Ziel-Updater und Installationszentrale teilen denselben root-eigenen Update-Lock; sichere Altmodi werden vor jeder Mutation normalisiert, ein belegter oder unsicherer Lock endet kontrolliert statt in einer Wiederholschleife. Bei einer Installation unter einem privaten Home-Pfad ergänzt der Updater auf echten benötigten Vorfahren ausschließlich das Traversierrecht; Auflisten, Lesen und Schreiben werden nicht freigegeben. Speziellere Rechte für Konfigurationen, Zugangsdaten und Laufzeitdaten bleiben erhalten; das gilt auch für den gemeinsamen Lesemodus `0664` der beibehaltenen `external_pv_topology.json`. Eine Matter-Kopplung lässt sich über einen bestätigten Bare-Metal- oder Docker-Weg zurücksetzen. Zufällige Stage-Reste sind unverbindlich; erst feste Prepare-, Quarantäne- und Receipt-Namen autorisieren die Fortsetzung, wobei das Parent-Receipt als zweiter Name desselben Marker-Inodes veröffentlicht wird. Update-, Rechte- und HA-Läufe schützen auch den Stage-Präfix. Bare Metal bindet Konfiguration, Unit und Eigentümervertrag nach dem Reset erneut; Nutzer-`Aus` gewinnt und ein Resetfehler nach dem Stop lässt Matter gestoppt. Docker zeigt keine systemd-Meldung. Historische Rechtefehler werden zuerst über den regulären Update-/Rechteweg behoben. Ältere Docker-Images ohne den neuen Resetvertrag bleiben lauffähig und verbrauchen diesen Auftrag nicht.

> **Weitere Korrekturen in 5.4.4f:** Gewöhnlicher Netzbezug wird zuerst bis zum physischen Mindeststrom ausgeregelt; erst der gemeinsame Wh-Integralvertrag darf danach einen Phasenwechsel oder Stopp auslösen. Ein paralleler Direct-0-A-Pfad kann diese Reihenfolge nicht umgehen. Die Direktvermarktungsansicht fasst semantisch gleiche Viertelstunden zusammen, lässt echte Preis-, Verkaufs- und Vertragsgrenzen sichtbar und markiert unbelegte Planlücken als `EVIDENCE_LIMIT`. Standard-PV-, Standard-SoC- und Tagessummen bleiben parallel sichtbar. Die zusätzliche DV-SoC-Linie ist rein lesend und integriert im bereits laufenden Slot nur dessen tatsächlich verbleibende Dauer; sie öffnet weder Befehlsfreigabe noch Hardwareausgang.

> **Startbudget der ruhigen PV-Kurve:** Der separat belegte batterieneutrale PV-Anteil wird nicht noch einmal durch einen Netzpunktwert verkleinert, den die gleichzeitige Speicheraufnahme bereits beeinflusst hat. Eine zentral freigegebene einphasige Ladung fällt dadurch nicht allein wegen dieses zusätzlichen Filters wieder unter ihre Mindestleistung.
...
> `E3DC_IMAGE_TAG=v5.4.4f` in `.env` gesetzt. `config --images` zeigt vor dem
