...
def _copy_matching_files(source_root, destination_root, extensions):
...
def backup_current_version():
    ...
    try:
        ...
        e3dc_control_dir = os.path.join(INSTALL_PATH, "E3DC-Control")
        e3dc_extra_backup_dir = os.path.join(backup_dir, "e3dc-control-extra")
        if os.path.isdir(e3dc_control_dir):
            ...
            _print_missing_source(e3dc_control_dir)
...
def restore_backup(backup_path):
    ...
    try:
        ...
        e3dc_control_dir = os.path.join(INSTALL_PATH, "E3DC-Control")
        if os.path.isdir(e3dc_extra_backup_dir) and os.path.isdir(e3dc_control_dir):
            restored_count = 0
            for root, _, files in os.walk(e3dc_extra_backup_dir):
                for filename in files:
                    src = os.path.join(root, filename)
                    rel_path = os.path.relpath(src, e3dc_extra_backup_dir)
                    dst = os.path.join(e3dc_control_dir, rel_path)
                    try:
