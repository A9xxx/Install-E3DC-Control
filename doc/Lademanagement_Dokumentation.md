...
## 2. Zuständigkeiten im laufenden Betrieb

Für Speicher und flexible Verbraucher gilt eine klare Zuständigkeit:
...
freigegebenen Ziel-SoC und nur mit einem sichtbaren aktiven Besitzer.

---

## 3. Feature: Wärme-Gestehungskosten (Thermischer Preis-Boost)
...
## 4. Feature: Quell-Erholung (Pausenmodus für Wärmepumpen)

Bei der **Quell-Erholung** wird die Wärmepumpe nicht
...
## 5. Feature: Universal SG-Ready & EVU-Sperre
...
## 6. Feature: Intelligentes Zielladen (Auto SoC)
...
*   **Fahrzeug-Vorlagen speichern (Gast-Fahrzeuge):** Für Autos ohne Cloud-Anbindung können im Lade-Menü neutrale Profile samt Kapazität und typischer Ladeleistung gespeichert werden. Das System berechnet Ladezeiten und Restreichweite dann profilspezifisch, selbst für reine Gast-Fahrzeuge.
...
Der native **Python Wallbox Manager** bietet folgende sichtbare Nutzer-Modi:
...
Externe Wallboxen koennen gesteuert oder nur gemessen werden:

* **openWB Pro:** Direkte Steuerung ueber `connect.php`, wenn E3DC-Control Master sein soll.
* **openWB Software 2.x:** Als `primary` regelt openWB selbst; E3DC-Control liest Messwerte oder schaltet bewusst den openWB-Lademodus per simpleAPI. Als `secondary` muss in openWB `Steuerungsmodus: secondary` und `Steuerung ueber Modbus als secondary: An` gesetzt sein.
* **go-e:** Steuerung ueber HTTP-API v2.
...
Pre-Dump, Quell-Erholung und die Ladekurve werden vom Storage Manager geführt. Aktive Entladung bleibt an eine sichtbare Pre-Dump-/Kurvenentscheidung und den dafür ausgewiesenen Besitzer gebunden. Andere Verbraucherpfade dürfen die aktuelle Speicherführung nicht übersteuern.

---

## 8. Prognoseunterstützung
Das Lademanagement kann historische Messwerte für statistische Verbrauchs- und Erzeugungsprognosen auswerten.

---

## 9. System-Autonomie (Survival Mode)
