...
function loadNativeWallboxPlannedPower($conf) {
    ...
    foreach ($files as $file => $fallbackWbId) {
        ...
        foreach ($decoded as $entry) {
            ...
            if ($wbId === 2 && isWallbox2ExplicitlyDisabledConfig($conf)) {
                continue;
            }
...
function forecastHeadroomProjectionEvidence($plan) {
    ...
    foreach ($source['slots'] as $slot) {
        ...
        $identityEncoded = forecastTrajectoryCanonicalJson($identity);
        ...
        $windowIdentityEncoded = forecastTrajectoryCanonicalJson([
            'schema' => 'direct_marketing_headroom_projection_plan_v1',
            'energy_basis' => 'stored_battery_energy_delta_before_discharge_loss_v1',
            'headroom_export_budget_id' => $budgetId,
        ...
        $segmentIdentityEncoded = forecastTrajectoryCanonicalJson([
            'schema' => 'direct_marketing_headroom_projection_plan_v1',
            'energy_basis' => 'stored_battery_energy_delta_before_discharge_loss_v1',
            'headroom_export_budget_id' => $budgetId,
...
function forecastHeadroomEnergyBindingValid($binding, $sidecarSlot, $projectedW, $batteryW) {
    ...
    foreach ($source['slots'] as $index => $slot) {
        ...
        if ($projectionOnlyMarker) {
            ...
            $runtimeHoldValid = $canonicalActiveMarker
                ...
                && forecastDirectMarketingActionBindingValid(
                    $plan,
                    $projection,
                    'CHARGE_BLOCK_WAIT',
                    0.0,
            ...
            if ($action !== 'HEADROOM_EXPORT'
                ...
                || forecastTrajectoryCanonicalJson($headroomProjection) !== forecastTrajectoryCanonicalJson($expectedBinding)
                || forecastTrajectoryCanonicalJson($projection['direct_marketing_headroom_projection'] ?? null) !== forecastTrajectoryCanonicalJson($expectedBinding)
                ...
                || !forecastHeadroomEnergyBindingValid($energyBinding, $headroomSource, $projectedW, $batteryW)
...
function forecastDirectMarketingSlotState($projection, $runtime, $runtimeMatchesSlot, $requiresPrice) {
    ...
    $authorizedChargeW = $runtimeMatchesSlot
        ...
        : 0.0;
    $chargeActions = ['PV_STORE', 'DV_CURVE_CHARGE'];
    $commandAction = $runtimeAction === 'ECONOMIC_EXPORT'
        || in_array($runtimeAction, $chargeActions, true);
    ...
    $planSelectionComplete = $selected
        ...
        && $planCommandsAllowed
        && $plannedW > 0.0
        && ($planAction === 'ECONOMIC_EXPORT' || in_array($planAction, $chargeActions, true))
        && is_string($windowId)
        && trim($windowId) !== ''
    ...
    $active = $selected
        ...
        && (
            ...
            || (in_array($runtimeAction, $chargeActions, true) && $authorizedChargeW > 0.0)
        );
    return [
        'candidate' => $candidate,
        ...
        'authorized_export_w' => $active && $runtimeAction === 'ECONOMIC_EXPORT'
            ? $authorizedExportW
            : 0.0,
        'authorized_charge_w' => $active && in_array($runtimeAction, $chargeActions, true)
            ? $authorizedChargeW
            : 0.0,
        'active' => $active,
...
function forecastDirectMarketingPlannedChargeW($state) {
    ...
    if (
        !empty($state['active'])
        && in_array(
            strtoupper((string)($state['runtime_action'] ?? '')),
            ['PV_STORE', 'DV_CURVE_CHARGE'],
            true
        )
    ) {
    ...
    $fullySelectedPvStore = !empty($state['selected'])
        ...
        && in_array(
            strtoupper((string)($state['plan_action'] ?? '')),
            ['PV_STORE', 'DV_CURVE_CHARGE'],
            true
        );
