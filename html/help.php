...
sudo docker compose logs --tail=80 e3dc-control</pre>
        ...
        <div class="col-12 faq-item" data-tags="5.4.6c stable wallbox phasen pv wärme luxtronik docker">
            <div class="card bg-card border-0 shadow-sm"><div class="card-body">
                <h5 class="card-title">Was verbessert Wartungsrelease 5.4.6c?</h5>
                <p>Feste Python-E3DC-Wallboxen können unbekannte Fahrzeuge mit einer begrenzten 6-A-Probe erkennen. Tatsächliche Phasenmessung, ein gemeinsames 40-Wh-Konto und die zentrale Leistungsfreigabe bestimmen den weiteren Lauf. Zweite Wallboxen und Phasenwechsel werden berücksichtigt.</p>
                <p>Luxtronik-Heizung und Warmwasser erhalten getrennte PV-Aufträge. Mindestlaufzeit, tatsächliche Leistungsannahme und getrennte Akku-/Netzgrenzen schützen die Wärmepumpe. Vor optionalem PV-Boost das elektrische Leistungsprofil und die erlaubten Überbrückungskontingente im Config Editor eintragen.</p>
                <p>Docker unterstützt zusätzlich bestimmte ältere Compose-Ausgaben und die Privilegienprüfung älterer Kernel. Zuerst den Host-Updater aktualisieren und die Sicherungs- und Wartungshinweise beachten.</p>
            </div></div>
        </div>

        <h4 class="mb-4 text-accent">Stable 5.4.6b: Docker-Start und Update</h4>
        <div class="col-12 faq-item" data-tags="5.4.6b stable docker start rechte compose update rückfall">
            <div class="card bg-card border-0 shadow-sm"><div class="card-body">
                <h5 class="card-title">Was verbessert Version 5.4.6b?</h5>
                <p>Das interne Docker-Laufzeitmodul bleibt nach dem Wechsel auf das unprivilegierte Konto erreichbar. Der Imagebau prüft die nötigen Verzeichnisrechte und den tatsächlichen Privilegienwechsel.</p>
                <p>Der Host-Updater übernimmt zusätzlich bekannte alte Compose-Vorlagen mit zwei oder fünf Standardvolumes und erhält gültige Web-Port- und Bindadressen. Scheitert auch ein Rückstart, stoppt er den eindeutig zugeordneten Rückfallcontainer und meldet einen unbestätigten Stillstand ausdrücklich.</p>
                <p>Vor dem Upgrade den Host-Updater aktualisieren und die Volumes bei gestopptem Container sichern. Unsichere Compose-Dateirechte müssen auf dem Host korrigiert werden. Die Wartungs- und Rückfallhinweise der Docker-Dokumentation bleiben maßgeblich.</p>
            </div></div>
        </div>
        ...
        <div class="col-12 faq-item" data-tags="5.4.5d stable docker start datenvolume rechte datenschutz wallbox">
            <div class="card bg-card border-0 shadow-sm"><div class="card-body">
                <h5 class="card-title"><span class="tag">5.4.5d</span> Was verbessert Wartungsrelease 5.4.5d beim Docker-Start?</h5>
                <p>Die Startprüfung betrifft den gemeinsamen Datenordner. Sie verlangte bisher ausschließlich <code>2775</code> und lehnte dadurch den regulären Standardschutz <code>2770</code> ab. Startprüfung und anschließende Rechteverwaltung verwenden jetzt denselben konfigurierten Datenschutzmodus.</p>
                <p>Sichere vorhandene Datenvolumes bleiben erhalten. Unsichere Eigentümer, Verknüpfungen oder Anforderungsdateien werden weiterhin abgewiesen. Fehlende Wallbox-Anforderungsdateien müssen nicht angelegt werden. Docker-Updates erfolgen über den dokumentierten Host-Helfer.</p>
            </div></div>
        </div>
        ...
        <h4 class="mb-4 text-accent"><i class="fas fa-shield-halved me-2"></i>Stable 5.4.4e: Webupdate mit RAM-Disk</h4>
        <div class="col-12 faq-item" data-tags="5.4.4e stable update ramdisk tmpfs rücklauf rechte ha shadow">
            <div class="card bg-card border-0 shadow-sm">
                <div class="card-body">
                    <h5 class="card-title">
                        <span class="tag">5.4.4e</span>
                        Was korrigiert das Stable-Release 5.4.4e?
                    </h5>
                    <ul>
                        <li><strong>Produktive RAM-Disk:</strong> Das exakt bestätigte <code>tmpfs</code> unter <code>/var/www/html/ramdisk</code> wird beim Webupdate als beabsichtigter Laufzeit-Mount akzeptiert. Eine ausdrücklich in der Release-Löschliste benannte Altdatei wird darin fd-gebunden über den eigenen Mountroot entfernt. Andere fremde oder unbestätigte Mounts bleiben gesperrt.</li>
                        <li><strong>Rücklaufrechte:</strong> Nach einem fehlgeschlagenen Dateiaustausch werden die bekannten Rechte von <code>data</code>, <code>logs</code>, <code>tmp</code> und <code>ramdisk</code> vor dem Neustart des vorherigen Dienstsatzes wiederhergestellt.</li>
                        <li><strong>Kein zusätzliches Zeit-Endgate:</strong> Master und Slave übergeben ihre aktivierten Regelungsdienste an den HA-Manager. Shadow-Systeme sowie zuvor aktive, aber deaktivierte Dienste werden direkt wiederhergestellt.</li>
                        <li><strong>Regelung unverändert:</strong> Speicher-, Wallbox-, Wärme-, Tarif- und Direktvermarktungslogik entsprechen 5.4.4d.</li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <h4 class="mb-4 text-accent"><i class="fas fa-shield-halved me-2"></i>Stable 5.4.3s: fehlender Backup-Root im Download-Bootstrap</h4>
        <div class="col-12 faq-item" data-tags="5.4.3s stable update bootstrap backup root sicherung dienste rechte health">
            <div class="card bg-card border-0 shadow-sm">
                <div class="card-body">
                    <h5 class="card-title">
                        <span class="tag">5.4.3s</span>
                        Was korrigiert das Stable-Release 5.4.3s?
                    </h5>
                    <ul>
                        <li><strong>Erster Sicherungspfad:</strong> Der administrative Root-Download-Bootstrap legt einen wirklich fehlenden kanonischen Backup-Root sicher an. Ein bereits vorhandener unsicherer oder widersprüchlicher Pfad bleibt gesperrt.</li>
                        <li><strong>Unveränderter Ablauf:</strong> Danach werden das Backup erstellt und geprüft, die Dienste gestoppt, Dateien und Rechte aktualisiert sowie die Dienste gestartet und ihr Zustand geprüft.</li>
                        <li><strong>Keine Regelungsänderung:</strong> EMS-, Direktvermarktungs-, Wallbox- und Hardwarelogik ändern sich nicht.</li>
                    </ul>
                </div>
            </div>
        </div>

        <h4 class="mb-4 text-accent"><i class="fas fa-shield-halved me-2"></i>Stable 5.4.3r: HA-Rollenanker aus gebundener Rolle und Peer</h4>
        <div class="col-12 faq-item" data-tags="5.4.3r stable update bootstrap ha rolle peer rollenanker backup writer">
            <div class="card bg-card border-0 shadow-sm">
                <div class="card-body">
                    <h5 class="card-title">
                        <span class="tag">5.4.3r</span>
                        Was korrigiert das Stable-Release 5.4.3r?
                    </h5>
                    <ul>
                        <li><strong>Gebundener HA-Verbund:</strong> Der ausdrücklich mit Rolle und Peer gebundene Download-Bootstrap darf einen wirklich fehlenden HA-Rollenanker aus dieser bereits bestehenden Bindung erzeugen.</li>
                        <li><strong>Sicherer Zeitpunkt:</strong> Die Erzeugung erfolgt erst nach verifiziertem Backup und bestätigter Writer-Ruhe.</li>
                        <li><strong>Keine Regelungsänderung:</strong> EMS-Regelung und Hardwareausgänge ändern sich nicht.</li>
                    </ul>
                </div>
            </div>
        </div>

        <h4 class="mb-4 text-accent"><i class="fas fa-shield-halved me-2"></i>Stable 5.4.3q: absoluter visudo-Pfad im Bootstrap-Finalizer</h4>
        <div class="col-12 faq-item" data-tags="5.4.3q stable update bootstrap visudo pfad finalizer">
            <div class="card bg-card border-0 shadow-sm">
                <div class="card-body">
                    <h5 class="card-title">
                        <span class="tag">5.4.3q</span>
                        Was korrigiert das Stable-Release 5.4.3q?
                    </h5>
                    <ul>
                        <li><strong>Eindeutiger Aufruf:</strong> Der Finalizer des administrativen Download-Bootstraps verwendet den absoluten Pfad <code>/usr/sbin/visudo</code>.</li>
                        <li><strong>Unabhängig vom Root-Pfad:</strong> Ein verkürzter Root-<code>PATH</code> verhindert den Abschluss dadurch nicht mehr.</li>
                        <li><strong>Keine Regelungsänderung:</strong> EMS-Regelung und Hardwareausgänge ändern sich nicht.</li>
                    </ul>
                </div>
            </div>
        ...
        <h4 class="mb-4 text-accent"><i class="fas fa-shield-halved me-2"></i>Stable 5.4.3p: Git-Metadaten beim Installationsbenutzer</h4>
        <div class="col-12 faq-item" data-tags="5.4.3p stable update bootstrap git eigentuemer installationsbenutzer">
            <div class="card bg-card border-0 shadow-sm">
                <div class="card-body">
                    <h5 class="card-title">
                        <span class="tag">5.4.3p</span>
                        Was korrigiert das Stable-Release 5.4.3p?
                    </h5>
                    <ul>
                        <li><strong>Passender Repository-Eigentümer:</strong> Der administrative Download-Bootstrap erzeugt die neue <code>.git</code>-Fläche als den zuvor eindeutig gebundenen Installationsbenutzer.</li>
                        <li><strong>Sicherheitsvertrag unverändert:</strong> Verifiziertes Backup, bestätigte Writer-Ruhe und sämtliche Safety-Gates bleiben verpflichtend.</li>
                        <li><strong>Keine Regelungsänderung:</strong> EMS-Regelung und Hardwareausgänge ändern sich nicht.</li>
                    </ul>
                </div>
            </div>
        ...
        <h4 class="mb-4 text-accent"><i class="fas fa-shield-halved me-2"></i>Stable 5.4.3o: robuster Altinstallations-Bootstrap und passiver DV-Ladeblock</h4>
        <div class="col-12 faq-item" data-tags="5.4.3o stable update bootstrap altinstallation git rechte systemd direktvermarktung ladeblock">
            <div class="card bg-card border-0 shadow-sm">
                <div class="card-body">
                    <h5 class="card-title">
                        <span class="tag">5.4.3o</span>
                        Was ändert das Stable-Release 5.4.3o?
                    </h5>
                    <ul>
                        <li><strong>Unabhängiger Rettungsweg:</strong> <code>e3dc-update-bootstrap</code> lädt den veröffentlichten Stable-Tag samt Commit in einen root-eigenen temporären Checkout. Der vorhandene Alt-Updater und seine <code>.git</code>-Metadaten liefern keine Autorität.</li>
                        <li><strong>Normalisierung erst nach Sicherung und Stillstand:</strong> Bekannte Release-Dateien, Rechte und Units werden erst nach verifiziertem Backup und bestätigter Writer-Ruhe auf den Zielzustand projiziert. Pfadflucht, Symlinks, Spezialdateien, zusätzliche Hardlinks, konkurrierende Updates sowie fehlgeschlagene Backup- oder Healthchecks bleiben harte Stopps.</li>
                        <li><strong>Kandidatloser passiver DV-Ladeblock:</strong> Ein bewusst fehlender diagnostischer Plankandidat verhindert den vollständig an Plan, Slot, DV-Owner und Ausgang gebundenen sicheren 0-W-Ladeblock nicht mehr. Laden oder Entladen wird dadurch nicht neu autorisiert.</li>
                    </ul>
                </div>
            </div>
        </div>

        <h4 class="mb-4 text-accent"><i class="fas fa-shield-halved me-2"></i>Stable 5.4.3n: pfadgenauer Rollenanker im Update-Rückweg</h4>
        <div class="col-12 faq-item" data-tags="5.4.3n stable update backup recovery restore rollenanker root www-data">
            <div class="card bg-card border-0 shadow-sm">
                <div class="card-body">
                    <h5 class="card-title">
                        <span class="tag">5.4.3n</span>
                        Was korrigiert das Stable-Release 5.4.3n?
                    </h5>
                    <ul>
                        <li><strong>Kanonischer Rollenanker:</strong> Ausschließlich <code>/etc/e3dc-control/instance_role.json</code> wird mit den vorgeschriebenen Metadaten <code>root:www-data 0640</code> als privilegierte Restorequelle akzeptiert.</li>
                        <li><strong>Private Sicherung:</strong> Die Backup-Payload bleibt <code>root:root 0600</code>. Alle anderen privilegierten Pfade sowie falsche Eigentümer, Gruppen, Modi, Links, ACLs, Attribute oder Identitätsdrift bleiben strikt fail-closed.</li>
                        <li><strong>Keine Regelungsänderung:</strong> EMS-, HA-, Wallbox-, Wärme- und Direktvermarktungsentscheidungen sowie Hardwareausgänge entsprechen unverändert 5.4.3m.</li>
                    </ul>
                </div>
            </div>
        </div>
        ...
        <h4 class="mb-4 text-accent"><i class="fas fa-shield-halved me-2"></i>Stable 5.4.3l: gebundener Update-Rückweg und sichere Kernunits</h4>
        <div class="col-12 faq-item" data-tags="5.4.3l stable update backup recovery rollback dirty systemd storage manager piguard auto restart startschutz">
            <div class="card bg-card border-0 shadow-sm">
                <div class="card-body">
                    <h5 class="card-title">
                        <span class="tag">5.4.3l</span>
                        Was korrigiert das Stable-Release 5.4.3l?
                    </h5>
                    <ul>
                        <li><strong>Git-gebundener Rückweg:</strong> Der native Ziel-Updater bindet Repository, Ausgangscommit, root-eigenes Transaktionsbackup und Transaktion vor der ersten Dienstmutation. Ein unsicherer oder ausgetauschter Sicherungspfad bleibt gesperrt.</li>
                        <li><strong>Belegte lokale Änderungen:</strong> Bei weiterhin vorhandenen, geänderten Git-Dateien stellt der Rückweg die gesicherten Bytes wieder her und härtet den Dateimodus auf den in <code>old_commit</code> belegten Git-Modus. Staged Indexstände, ungetrackte oder gelöschte Dateien und allgemeine manuelle Restorepfade sind davon nicht neu abgedeckt.</li>
                        <li><strong>Sichere Kernunits:</strong> Nur eine exakt freigegebene ältere Storage-Manager-Unit darf vor dem ersten Dienststopp atomar in den root-eigenen Vertrag migriert werden. PiGuard mit <code>activating/auto-restart</code> wird als zuvor laufender Wächter erfasst; abweichende oder unklare Zustände bleiben fail-closed.</li>
                        <li><strong>Erkannter Recoveryfehler:</strong> Scheitert der updater-eigene Rückweg synchron und nachweisbar, bleibt ein transaktionsgebundener Startschutz für PiGuard und die bekannten Writer bestehen. Der nächste normale Updateversuch stoppt vor einer Mutation, solange dieser Schutz vorhanden oder nur teilweise belegbar ist.</li>
                        <li><strong>Evidenzgrenze:</strong> Der Startschutz ist keine pauschale Zusage für Stromausfall, <code>SIGKILL</code> oder einen Prozessabbruch außerhalb des erkannten Fehlerpfads. HA-, Wallbox-, Speicher-, Wärme- und Direktvermarktungsentscheidungen ändern sich nicht.</li>
                    </ul>
                </div>
            </div>
        </div>
